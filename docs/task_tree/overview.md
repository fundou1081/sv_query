...
| **125** | **2026-09-08** | **L1** | **POC 语料补全 — 5 个新失败清零 (iter_223, goal 轮 1)** | goal-e8196ce3 | **37 -> 32 (持平基线)** | **worktree 对照法精确定位 (git worktree add f78e7d2 + FAILED 差集) -> 5 个新失败全在 poc/test_portconn_native_poc.py; 真因: 测试语料清单不全 (darkspi.v 实例化 spi_master, 定义在同语料 rtl/lib/spi/spi_master.v, 实测存在), 过去靠 API 级 strict=False 吞掉 "unknown module"; 修: 文件清单补 lib/spi/spi_master.v -> 该文件 5 passed; 全量 **32 failed / 3283 passed** 与基线持平** | **✅ 持平** |
| **124** | **2026-09-08** | **L1** | **逐文件工具 + 全量改写 202->27 (iter_222)** | 方豆: 继续做, 直到全部完成 (goal) | **⚠️ 175 处已移除 / 37 failed (5 新)** | **新增 tools/remove_strict_in_file.py (单文件 AST 精确改写: 形参/关键字实参/**位置实参**/if 分支; 默认拒绝核心 self._strict); 改写 **80 文件**, 202 -> **27 处** (位置实参 0); 冒烟 10 命令 9/10 rc=0 (controlflow 那条是我命令拼写); 全量 **37 failed / 3278 passed** (32->37 恶化 5) -> 按"用户要求全改+失败先保留"保留并记录; 剩余 27 处在 7 个文件 (主要核心 self._strict, 工具按设计跳过)** | **⚠️ 待修 5 个新失败** |
| **123** | **2026-09-08** | **L1** | **逐文件改造 文件 1/N: handshake.py (iter_221)** | 方豆: 逐文件修改, 全改 | **✅ 全量改善 34->32** | **方案 A 首个文件 (含全仓唯一位置实参): AST 精确删 2 形参 + 6 关键字实参 + **1 位置实参** + 残余 if strict; 五层验证全过 (ast.parse / scan_strict 该文件 0 / 冒烟 handshake scan+analyze rc=0 / 相关测试 297 passed / **全量 32 failed 3284 passed**, 比批次 2 的 34 改善 2); 证明"逐文件一次提交"节奏可行 (对比 5 次"一次改一层"全回退)** | **✅ 文件 1 完成** |
| **122** | **2026-09-08** | **L1** | **AST 驱动彻底改造尝试 -> 仍破坏 CLI, 已回退 (第 5 次) (iter_220)** | 方豆: 进行彻底改造 | **❌ 回退 + 战略结论** | **技术最正确的一次 (AST 精确区间: 删 120 关键字实参 + 18 形参 + **1 位置实参**, 复测位置实参 1->0); 但冒烟 5/6 rc=1 -> 回退; 最可能因: 删形参后**函数体仍引用 strict** (裸引用 12 / self._strict 7) 与**多层包装调用链未同步**; **战略结论: strict 是横切关注点, 任何"一次改一层"都会留未覆盖路径 -> 改为"一个文件一次提交"(每次 scan_strict 复测 + 该文件用例 + 全量门禁, 绿才提交); 另建议选项 C: 先修 34 个失败 (严格模式暴露的 fixture 真错) 再移除 strict** | **❌ 回退 (战略已定)** |
| **121** | **2026-09-08** | **L1** | **写 AST 扫描工具 scan_strict.py — strict 移除精确清单 (iter_219)** | 方豆: 可以，去做吧 | **工具落地 + 202 处清单** | **AST 分类统计: 形参 22 / 关键字实参 160 / **位置实参 1** (handshake.py:311 ⚠️) / 裸引用 12 / self._strict 7 = 202 处 61 文件; grep 对账 471 行 (含注释/字符串, 合理超集 → 不能按 grep 数字判断进度); **关键: 唯一 1 处位置实参即可解释 iter_218 的 116 failed** (删形参后位置整体错位 → 静默行为错), 而这正是正则方法的盲点; 下一步配方: 先清位置实参 → 逐文件删形参与实参 (每文件用工具复测 + 跑相关用例) → 核心层 self._strict 单独立项** | **✅ 工具就绪** |
| **120** | **2026-09-08** | **L1** | **批次 3a: 冒烟过但全量恶化 34->116, 已回退 (iter_218)** | 方豆: 按这个继续做 | **❌ 回退 + 换方法** | **第一次范围过大删到 UnifiedTracer 形参 -> NameError; 第二次限定 src/cli (strict=True 111->0) **冒烟 5/5 rc=0**, 但全量 **116 failed** (批次 2 是 34) -> 恶化 82; 从备份回退, 复核 15 passed + 工作树干净; **第 4 次同类教训: 冒烟 != 安全, 改签名必须跑全量; 根因是用正则做本应逐点审查的调用点改造 -> 下次改用 AST 枚举 + 逐文件验证 + 位置传参单独检查** | **❌ 回退 (方法已换)** |
| **119** | **2026-09-08** | **L1** | **批次 3 (API 形参) 尝试 -> 行内传参遗漏, 已回退 (iter_217)** | 方豆: 继续推进 | **⚠️ 3a 有效 / 整体回退** | **3a (CLI helper 去形参 + 删 88 处死传参 + handle_compilation_error 去形参) 冒烟全绿; 但遗漏**行内**形态 `strict=True, other=...` -> `_build_tracer` 已无形参 -> TypeError -> **265 failed / 6 errors**; 从备份回退到批次 2 状态 (34 failed / 3282 passed, 复核 stats/trace rc=0 + test_design 10 passed); 教训 (第 3 次同类): 删命名参数必须穷举 3 种写法并用 grep -c 对账; 核心层 `self._strict` 是**语义分支**不是传参, 需单独立项** | **⚠️ 回退 (配方已就绪)** |
| **118** | **2026-09-08** | **L1** | **彻底移除 strict 批次 2: 删全部 34 个 CLI 选项 (iter_216)** | 方豆: 推进批次2, 全部移除 | **CLI 选项清零 / 全量 34 failed 待修** | **删 30 处选项定义 + 常量 (STRICT_OPTION) + 20+ 文件 import + 参数行 + 114 处 strict=strict -> strict=True; 三次自伤纠正 (常量删除致 ImportError -> CLI 套件 342 failed/13 errors; 测试显式传 --strict -> 19 failed) 均已修; 验证: `grep --strict/--no-strict src` = 0, 采样命令 --help 无 strict, stats/trace 冒烟 rc=0; **全量 canonical 34 failed / 3282 passed** (16 可视化暂缓 + 一批 fixture 严格模式下暴露真错, 如 snapshot 组 4 个); 待批 3 (API 形参) / 4 (生产 strict=False) / 5 (测试 API 级 29 处 + fixture)** | **✅ 批次 2 / ⚠️ 34 红待修** |
| **117** | **2026-09-08** | **L1** | **彻底移除 strict 批次 1: design.py (iter_215)** | 方豆: 去除默认值, 彻底移除 strict | **7 处 flag 追加清零** | **删 7 处 args.append("--no-strict") + 7 个辅助函数 strict 形参 + 顶层 strict 选项 + 位置参数调用; JSON "strict" 字段保留恒 True (兼容下游); 验证 design show rc=0 + test_design.py 10 passed; 全仓 src/** 的 --no-strict 用法 = 0; **教训: 删选项要同步删所有引用 (残留会 NameError), 靠 grep + 冒烟跑而非只信 ast.parse; "用法归零"结论必须全路径 grep (iter_211 只扫了测试目录)** | **✅ 批次 1** |
| **116** | **2026-09-08** | **L1** | **strict 降级全仓扫描 (iter_214)** | 方豆: 好，先扫 | **44 处地图 (未改代码)** | **A. CLI 选项默认非严格 4 处** (arch:897 / backpressure:351 / coverage:232 / design:372, help 里甚至写明 default non-strict); **B. API 默认 strict=False 2 处** (_evidence_helpers:28 / coverage_gen_demo:831); **C. 生产与脚本调用点 9 处** (fix_imports/arch x2/fix x2/fix_widths/snapshot x2 + tools/fix_timescale); **D. 测试里 API 级降级 29 处** (iter_211 只清了命令行字符串); E. 误报 4 处 (注释/docstring/无关函数); 关键: **不传 flag 就是降级** (4 个命令) 比显式用法更危险 | **✅ 清单完成** |
| **115** | **2026-09-08** | **L1** | **fixture 修好 + 发现"降级默认值"桶 (iter_213)** | 方豆: 可以, 做吧 | **28 passed / 下一桶已定位** | **fixture `sim/test_comprehensive.sv` 改 `output logic` (10 处) -> 19 个 AssignToNet 清零; 原 TestNoStrictFlag 改写为正向断言 -> test_coverage_gen_demo + test_coverage_generate **28 passed**; **澄清**: iter_212 的"24 个新失败"不是 fixture 引起 (只有 1 个文件引用它), 而是**默认值层**: `coverage.py:232` 的 `strict=False` CLI 默认 + tools 脚本默认 -> 改严格后 15 failed (其 fixture 严格模式下有错); 回退工具改动, 记录为下一桶; **违规三形态**: 用法(已清)/默认值(新发现)/空转参数** | **✅ ①② 完成 / ③ 记录** |
| **114** | **2026-09-09** | **L1** | **清除全部 --no-strict 违规写法 (iter_211)** | 方豆: 先清掉所有, 然后再修; 失败就先保持失败状态 | **0 违规 / 遗 18 failed** | **sim/tests 与 tools 的 --no-strict **用法归零**; 3 个"专测该 flag"的文件**移出归档** (docs/archive/2026-09-09-nostrict-cleanup/ + README); CLI 选项保留(用户逃生舱); 全量 canonical **18 failed / 3298 passed**, 失败集中在 2 文件 (test_visualize_teach_nested_mux 16 = fixture 有真实 elab 错误; test_coverage_gen_demo 2); **这正是 flag 制造的"假绿"**; 下一步统一修这 18 个** | **⚠️ 清理完成 / 待修失败** |
| **113** | **2026-09-08** | **L1** | **清理测试里的 --no-strict 第一批 (iter_210)** | 方豆: 使用 no strict 明确违反开发纪律, 不可接受, 必须更改 | **111 处已清 / 91 处待处置** | **实测 sim/tests 共 202 处 / 41 文件; 机械清理 25 文件 122 处 (逐文件 ast.parse 校验, 12 复杂文件安全跳过); 撤后 unit+cli 暴露 **16 failed 全集中在一个文件** (test_visualize_teach_nested_mux.py) -> 该文件过去靠 flag 容忍 fixture 的真实 elaboration 错误 -> 已回退该文件并登记待修 fixture; 其余 24 文件撤后全绿 (flag 本来多余); **全量 canonical 3337 passed / 0 failed**; 剩余分类: 3 个 flag-behavior 测试文件(35 处, 建议归档) + 该 fixture 文件(7) + 复杂文件与 tools 脚本** | **⚠️ 第一批完成** |
| **112** | **2026-09-08** | **L2** | **R4-1 落地: filelist 经 -f 传入给明确提示 (iter_209)** | 方豆 好，去做吧 | **1 修 + 3 锁** | **推翻上轮结论: 守卫**当时就生效** — "只看到路径"是我自己调试打印 `l[:100]` 造成的假象 (路径恰好 ~100 字符, 正文被截断); `handle_compilation_error` 也没吞消息; 真正问题: 提示自相矛盾 (先"看起来是 filelist"再"建议 --no-strict") -> 按错误类型分支; 判据含 iter_208 两条教训 (扫前 20 有效行 / 裸路径须无空白无分号); 实测误用 → 明确提示且无 --no-strict; 合法 .v/.sv 语料不误判** | **✅ R4-1 闭环** |
| **111** | **2026-09-08** | **L2** | **R4-1 尝试与回退 (iter_208)** | 方豆 可以，继续做吧 | **❌ 未生效 + 2 线索** | **原 R4-1 场景已被 R4-3 顺带修好 (stats -f <带+incdir+ filelist> 现 rc=0); 剩余: 非 .f 扩展名 filelist 经 -f 传入仍给误导提示; 守卫扩展尝试失败三点: ① 判据不能只看首行 (预处理器注入 `timescale 使 +incdir+ 退到第 2 行) ② 裸路径判据要收紧 (endswith(.v) 会把 `endmodule // x.v` 误判) ③ **守卫信息被 handle_compilation_error 格式化吞掉** (用户只看到 Error: <path> + 通用 Hint) -> 已回退, 线索留档** | **❌ 回退 (线索已留)** |
| **110** | **2026-09-08** | **L2** | **R4-3 落地: filelist +incdir+ 传入编译器 (iter_207)** | 方豆 好，按这个来做 | **修复落地 + 2 锁** | **新增 `_read_filelist_full()` 交出 spec.include_dirs + `_build_tracer` 合并 incdir; 实测 graph/stats --filelist 从 rc=1(Undeclared 级联) -> **rc=0**, trace 与简单 include 无回归; 关键: **先修 R4-4 再做 R4-3** 的顺序是对的 (iter_205 卡在既有红上, 无法区分新旧问题)** | **✅ R4-3 闭环** |
| **109** | **2026-09-08** | **L2** | **R4-4 修复: 输入路径显示形态不一致 (iter_206)** | 方豆 好 push完再继续 | **9 failed -> 0 + 11 处去重** | **更正上轮判断: 全量 canonical 实测同样 9 failed -> 不是测试隔离, 是 HEAD 真实既有失败 (`--file` 显示 /var/... 而 `--filelist` 显示 /private/var/..., macOS 符号链接 + iter_193 后两侧 resolve 不一致); 通用修复: 新增 `cli._common.display_path()` 并收敛 risk/cdc/sva/timing/verify/controlflow 共 **11 处**重复显示逻辑; 断言未动 (改的是产品行为); 该文件 9 failed -> **15 passed**; 教训: 局部/全量的结论必须用当期实测确认, 不能引用上轮门禁的记忆** | **✅ R4-4 闭环** |
| **108** | **2026-09-08** | **L2** | **R4-3 修复: filelist +incdir+ 被丢弃 (iter_205)** | 方豆 先修吧 | **真因定位 ✅ / 修复回退 / R4-4** | **调查链: 纯 pyslang 0 错误 -> 我们的编译器(原文/预处理)均通过 -> CLI 路径对照 (graph/stats 失败 vs trace 正常) -> 定位 `_build_tracer`: filelist 路径 `_read_filelist()` 只取 sources, iter_193 已解析出的 spec.include_dirs 被丢弃 -> `+incdir+` 从未生效; 症状(token paste 不生效)误导方向, 真因是 include 目录丢失 -> 头文件宏无法展开 -> 级联 UndeclaredIdentifier; 修复: `_read_filelist_full()` 交出 include_dirs + `_build_tracer` 合并 (trace 路径不受影响); 实测 graph/stats rc=1->0; 4 条回归测试; R4-2 结论更正: 我们预处理器只支持 object-like 宏, 含参宏由 pyslang 展开, 不构成 bug** | **⚠️ 修复已回退 + 新发现 R4-4** |
| **107** | **2026-09-08** | **L2** | **对抗第四轮: 跨文件 include/宏/filelist (iter_204)** | 方豆 "继续" | **2 发现 (待修)** | **通过: include+incdir / 跨文件 define / 嵌套 filelist / 缺失条目 / 自包含 include 循环; **R4-2 真 bug: 宏 token paste (`nm``_q`) 不生效** → 3 行最小复现 + 对照实验 (同样链路不含 `` 时 rc=0) 定位到**宏展开**而非 include; **R4-1 UX 缺口: `-f <filelist>` 误用不再触发 iter_189 守卫的清晰提示** (filelist 内容根节点不是表达式 → 判据不覆盖) → 退化成下游 UndeclaredIdentifier + "Use --no-strict" 误导信息; 建议: 预处理器处理 `` + 守卫判据扩展到 filelist 特征** | **⚠️ 待修 (方案已列)** |
| **106** | **2026-09-08** | **L2** | **对抗第三轮: 分析层 + F6 (iter_203)** | 方豆 "继续" | **1 真 bug 修 + 3 回归锁** | **分析层 10 用例健壮 (generate 双 genvar 嵌套 / 参数化 class extends + interface modport / 宏拼接名实例 / 重复 module / 多语言料 filelist, 无崩溃无假成功); **F6: `trace --format json` 输出被 JSON 转义的**字符串**而非 JSON 对象** (实测 `"Fanin of '...':\n (no drivers)\n"`), 而 `--json` 正确 → 别名未收敛; 修复: 三处 dispatch 补 `elif format == "json": output_json(data)` (与 --json 同实现); 教训: "看着像 JSON" 比"不是 JSON"更危险 (json.dumps(文本) 可被 loads 解析但类型是 str) → 测试必须断类型** | **✅ F6 闭环** |
| **105** | **2026-09-08** | **L2** | **对抗测试第二轮: --quiet 契约违背 F5 (iter_202)** | 方豆 "继续" | **1 修 + 3 回归锁** | **换面打 quiet × JSON × stdout 纯净性 + 短选项 -j + 退出码一致性; 通过项: --quiet/-j/SVQ_QUIET 组合仍正确给结构化信封、未知 flag rc=2 干净、错误不被 quiet 吞; **F5 发现: `--quiet` 实测残留 225 字符 6 行 stderr** (Phase 3/4 进度 + pipeline 摘要, 全部绕过日志系统直接 print) → 违背 "--quiet 抑制所有 stderr" 契约; 修复: 新增 `compiler.is_quiet()` 让输出点自门控 + 6 行加门控 → `--quiet` stderr **0 字节** (默认仍 877)** | **✅ F5 闭环** |
| **104** | **2026-09-08** | **L2** | **JSON 契约 F1~F4 修复 (iter_201)** | **方豆 "先把这几个修了"** | **4 修 + 8 回归锁** | **F1 新增 `emit_json_error` → 错误也打 `{ok:false, command, error:{type,message}}`; **必须挂两层** (命令内 except + CLI 顶层, 因为文件/编码错误会逃到顶层); F2/F3 `warn_flags_ignored_by_json` → `--json` 与 --svg/--timing/--load-path/--unfold 组合时显式告警且不产出文件; F4 `--max-paths` 负值 → typer.BadParameter (rc=2), 0 仍合法; 新增 8 条对抗回归测试 (F1×3+反向 / F2F3×2 / F4 正反)** | **✅ 4 个问题闭环** |
| **103** | **2026-09-08** | **L2** | **对抗性测试: JSON 契约 (iter_200)** | **方豆 "来做一些对抗性测试，找到现有功能的问题"** | **4 个问题** | **F1 契约缺口: `--json` 错误路径 stdout **空**(rc=1 只有人读 stderr) — 成功信封有 ok 却无错误信封 (4 用例命中); F2 `--json --svg X` 静默丢弃 SVG (rc=0 无产物无提示); F3 `--json --timing` 静默忽略 timing 语义; F4 `--max-paths -1` 静默当 0 (无报错无其他语义); 根因: 成功契约≠完整契约 + 模式互斥无校验 + 参数校验缺失; 建议: 错误也打 `{ok:false,...,error}` / 互斥 flag 显式告警 / 负值报错 + 把 14 组矩阵固化为回归测试** | **⚠️ 待修 (方案已列)** |
| **102** | **2026-09-08** | **L2** | **timing analyze --json 字段级验收测试 (iter_199)** | **方豆 "按你的推荐，补测吧"** | **8 条验收 (同模板)** | **实测字段: total_nodes/reg_count/critical_paths[{depth,score,registers,full_path}]; 新增 8 条测试 (纯 stdout / 信封与 pipeline 一致 / result 与嵌套 schema 固定 / reg_count<=total_nodes / 每 path depth,score>=1 且 registers⊆full_path / --max-paths 生效 / reg_count==0 时 critical_paths 为空); **不把数值写死** (吸取 iter_195 `rankdir=LR` 教训: 别把实现细节当契约); 模板复用 → 两个命令信封统一 `{ok,command,result}`** | **✅ 标准生效** |
| **101** | **2026-09-08** | **L2** | **pipeline 结构化输出落地 --json (iter_198)** | **方豆 "嗯，去做吧"** | **验收标准生效** | **`visualize pipeline --json` 落地 (=dataclasses.asdict + 与 timing 对齐的信封 `{ok,command,result}`; 诊断仍走 stderr → stdout 纯 JSON); 不渲染可视化 (按决策冻结); 新增 7 条字段级验收测试 (stdout 纯 JSON / 信封一致 / result 字段固定 / stages 字段固定 / 寄存器三分类互斥 / stage_count==len(stages) 且 latency>=1 / 深链 latency 不变量) → 通过; 实测 cordic 语料 total_latency=1 stage_count=1 pipeline_regs=45** | **✅ 新标准生效** |
| **100** | **2026-09-08** | **L2** | **文本结构化输出审计 pipeline/timing (iter_197)** | **方豆 "所有输出以文本结构化输出为检查标准"** | **差距定位 + 实现路径** | **审计: `timing analyze` **已有 `--json`** (信封 `{ok, command, result}`); `visualize pipeline` **完全没有结构化输出** (仅 stderr 人类可读行 Pipeline regs/Control regs/State regs/Stages) → 主要缺口; 实现路径已定位: `pipeline_viz.detect_pipeline()` 已返回 `PipelineInfo{module_name, stages, total_latency, pipeline_regs, control_regs, state_regs}` → `--json` 只需 asdict + 对齐 timing 信封 (诊断走 stderr 保证 stdout 纯 JSON); 本轮**未改代码**, 审计兼实现计划落档** | **✅ 审计完成** |
| **99** | **2026-09-08** | **L2** | **可视化 flag 统一为 --svg (iter_196, 方豆决策)** | **方豆 "可视化 flag 统一改为 svg, 不再支持 dot"** | **--dot 全仓清零 + 全绿** | **7 个 SVG 子命令的 `--svg/--dot/-d` 别名收敛为 `--svg`; 真 DOT 输出的 5 个命令 (module/teach/datapath/timing/verify) `--dot`→`--emit-dot` (换名保留能力); 测试按**命令边界**映射 57 处 (全局 sed 会静默改行为); 文档示例同步 (历史迭代记录不改); **关键发现: `--svg` 在 chain 里被声明两次** (内部渲染器 vs 外部 graphviz) → 旧代码把 `--svg` 绑给后者, 而测试靠 deprecated `--dot` 别名才命中内部渲染器 (别名掩盖真缺陷); 修复: graphviz 那条改名 `--svg-graphviz`; 试做时只改 CLI → 52 failed, 遂一次做完 CLI+测试+全量验证; 结果 `grep --dot src sim/tests` = 0, 全量 3302 passed** | **✅ 契约统一** |
| **98** | **2026-09-08** | **L2** | **viz 断言迁移决策就绪 + 上游 issue 文本 (iter_195)** | **方豆 "继续" (iter_188 的 13 skip)** | **决策表 (未改断言)** | **实测每条 skip 的原意图 vs 当前 SVG 能否验证: 可直接迁移 3~4 条 (chain 异常类有 polygon+颜色+图例文本; trace "0 loads" 应断 stdout) / 需先定产品语义 7~8 条 (SVG 无 stage/control/critical 文本) / 需先补 CLI 能力 2 条 (pipeline 无 --png); 额外发现: 原 `rankdir=LR` 断言**早已不成立** (实测宽高比 0.28 竖版); 根因 = 同一 CLI 内 `--dot` 三种语义 (module=DOT / pipeline,chain=SVG / trace 无 --dot); 交付 ADR `ventus_viz_assertion_migration.md` 三问 + iter_189 追加可提交的上游 issue 文本** | **✅ 决策就绪** |
| **97** | **2026-09-08** | **L2** | **空 filelist 报错 + reclaim 改 opt-in (iter_194)** | **方豆 "继续" (iter_190~193 未决项)** | **两项小决策落地** | **① 空 filelist (一个文件都解析不出) 过去只告警 → 命令输出**空图且 rc=0** (静默失败); 两侧消费方现均抛 `CompilationError` (前提: iter_193 已统一解析规则, "空"才是真空) → 链路实测 rc=0 空图 → **rc=1 + 一行错误**; ② `reclaim_memory()` (4GB 技巧, iter_185 后无必要) 默认关闭、保留 `--reclaim` (移出代替删除原则 + 极端内存环境仍可用)** | **✅ 契约收紧 + 默认更快** |
| **96** | **2026-09-08** | **L2** | **合并两套 filelist 加载器 (iter_193)** | **方豆 "先合并 filelist"** | **方案 A: 纯解析器** | **抽出 `trace/core/filelist.py::parse_filelist()` (FilelistSpec{files,include_dirs,defines,missing}); 两侧差异实测为**两处** (相对路径基准 + 嵌套 -f 基准) → 统一为 "filelist 目录 → base_dirs" 候选; tracer/CLI 两个消费方变薄封装 (API 签名兼容); 实测两侧一致 (1/1, 1/1); 新增 parity 测试 5 个把"两侧文件集一致"变成不变量; 自伤: parity 测试初版传错基准 → 诊断出生产里第二基准都是 cwd, 改 chdir 而非改断言** | **✅ 单一实现** |
| **95** | **2026-09-08** | **L2** | **filelist 相对路径双基准解析 (iter_192)** | **方豆 "继续" (iter_190 未决项)** | **分歧消除 + 安全网** | **实测两套加载器规则不同 (tracer 侧按 filelist 目录 / CLI 侧按 base_dir) → 同一 filelist 结果不一致, 且 CLI 侧对**实际存在**的文件误报"条目不存在" (iter_190 告警变噪声); 仓库内两种约定并存 (industrial_filelists 用仓库根相对) → 只能多候选解析: filelist 目录优先, 再 base_dir, 都不中才告警 (告警里列出候选基准); 实测 filelist 目录相对 CLI 0→1 / 仓库根相对 0→1; 回归锁 4 测试把两种约定 + 缺失情形钉死 (将来统一加载器的安全网); 自伤记录: 测试里日志格式化写错 TypeError 已修** | **✅ 一致 + 有网** |
| **94** | **2026-09-08** | **L2** | **CLI 顶层统一错误格式化 — 垃圾输入不再甩 traceback (iter_191)** | **方豆 "继续" (iter_190 敌意输入矩阵遗留)** | **7 非法 × 3 子命令 → 干净报错 + 40 测试** | **三层边界问题收尾: iter_189 崩进程 / iter_190 静默 / iter_191 不可读; 新增 `main.run()` 统一入口 (只格式化 OSError 家族 + UnicodeDecodeError + CompilationError; 其他异常照旧抛 = 不掩盖真 bug; `SVQ_DEBUG=1` 恢复 traceback), `run_cli.py` 与 console script 共用 (过去各自 app() 容易漂移); 结果: 二进制/UTF-16/目录/不存在/filelist 当源码 全部 `sv_query: error: <一行原因>` + rc=1, traceback 全消失; 回归锁 40 测试 (三条不变量: 不因信号死 / 无 traceback / 非法必须非 0) + **反向断言** 合法极端输入 (空文件/仅注释/define/BOM+CRLF) 必须 rc=0** | **✅ 边界可读** |
| **93** | **2026-09-08** | **L1** | **`except: pass` 全仓清算 + 机械检查器 (iter_190)** | **方豆 "继续" (查 CLI 敌意输入时发现)** | **52 → 0 + 可执行检查** | **AGENTS v1.4 声称计数=0, 实测 **52 处** (25 处 `except Exception: pass`) → 声明无机械保障就会长回来; 52 处全部改为可见日志 (核心路径 debug / CLI warning; 理由注释保留; 9 文件补 logger); 新增 `tools/check_except_pass.py` (违规=pass 任意类型; 允许=收窄类型+`...`+注释) + 纪律测试 (含检查器自检); 连带修 filelist 静默缺陷: 缺失条目/读失败/嵌套 filelist 缺失/语法错误 -f 行 **全部改为可见告警** (过去用户会拿到'少文件的图'以为完整); 自伤如实记录: 脚本化改写第 1 版留 pass、第 2 版删 pass 造成 10 文件语法错误、第 3 版丢注释 → 均从 /tmp 备份回滚重做** | **✅ 纪律可执行化** |
| **92** | **2026-09-08** | **L1** | **pyslang addSyntaxTree 原生 SIGTRAP — 根因 + 守卫 (iter_189)** | **方豆 "继续" (iter_188 发现的崩溃)** | **纯 pyslang 最小复现 + 边界守卫** | **二分: 与扩展名无关, 与"内容解析成什么"有关 → 纯 pyslang 5 行复现: `Compilation.addSyntaxTree()` 对**表达式根**语法树 SIGTRAP (filelist 内容被 slang script 模式解析成 DivideExpression); 逐步二分只有 addSyntaxTree 崩 (faulthandler 对 SIGTRAP 无效 → 二分比 lldb 快); 合法根类型实测: CompilationUnit (空/注释/define/多成员) / ModuleDeclaration (单 module) / ClassDeclaration; 修复: `_reject_non_design_unit` 只拒表达式根 (不误伤) → 报可行动 CompilationError; 端到端 `visualize module -f <filelist>` rc=-5(无输出) → **rc=1 + 明确提示**; 回归锁 11 测试, 守卫失效时 pytest 自己 exit=133** | **✅ 崩溃闭环** |
| **91** | **2026-09-08** | **L2** | **Ventus viz 套件分诊 — 常红转有信号 (iter_188)** | **方豆 "继续" (iter_187 顺带发现立项)** | **14 failed → 0 failed** | **四类根因: ① `--dot` 自 V100 起是 `--svg` 别名 (输出 SVG), 断言仍按旧 DOT 语义 (实测 SVG 无 rankdir/digraph/cluster) → 统一 `_read_dot()` 内容判定 + 明确 skip 原因; ② trace 子命令无 `--dot` (正确: `--format dot --output`); ③ `_ensure_sched_dots()` 静默吞 rc + 全程用被禁的 `--no-strict` (10 处 → 0) → 记录失败原因并上抛到 skip; ④ **新发现真 bug: `visualize module` 原生崩溃 SIGTRAP (rc=-5, 任意 target), 已用 git worktree 在 iter_183 复现证明非本次引入**; 结果: 本文件 15 passed / 13 明确 skip / 0 failed, opensource 子集 111 passed / 14 skipped / 0 failed (原 110/14 failed)** | **✅ 有信号 + 1 真 bug 登记** |
| **90** | **2026-09-08** | **L2** | **baseline 漂移清算 + 输入统一 + 漂移守卫 (iter_187)** | **方豆 "继续" (回审补偿措施)** | **3 baseline 重生成 + 2 新工具** | **挖出结构性缺陷: 3 个 baseline 全部与当前行为不符且不可复现 — 都采集于 iter_145 (top_modules) 之前 (free-floating 虚高): picorv32 708→438 / IM 2→0; verilog-axi 8,221→715 / IM 51→6; pulp_axi_xbar target 已不存在 (不可复现); 旧断言 `600<=nodes<=800` 只检查**文件自身** → 过时 baseline 一路绿灯, check_regression 对用户报假 regression; 新增 tools/benchmark/inputs.py (唯一输入构建点) + regen_baselines.py (重生成 + `--check` 漂移检测 rc=2); 重生成后 stdev 全 0.0; 断言收紧: picorv32 活体==baseline / flakiness ratio==1.0 且 stdev==0.0; 连带修 `test_benchmark_regression` 硬编码旧数值 (改比例派生 + IM 场景换 verilog_axi)** | **✅ 漂移清算 + 有守卫** |
| **89** | **2026-09-08** | **L1** | **同族隐患系统排查 + 生命周期回归锁 (iter_186)** | **方豆 "继续" (兑现 iter_185 报告建议)** | **2 真实隐患修复 + 4 回归锁** | **7 类扫描模式 (SourceManager 创建 / string_view option 赋值 / 符号逃出帧 / Compilation 创建 / 缓存存对象 / 硬编码路径 / 编译后改 compiler); 修复: ① d1 测试 helper `_compile_case27` 把 manager 丢在帧里却返回符号 → 隔离实测 top.name **3/3 UnicodeDecodeError** (套件一直靠内存未被覆写的运气过, 即 iter_158 "symbol 对象 str 垃圾" 来源) ② uvm_testbench_extractor `_class_defs` 存 syntax node 逃逸 → 显式持有 manager+compilation 至遍历结束, 并更正 "SVCompiler 污染 token.name" 错误注释 ③ `PYSLLANG_BINDINGS_PATH` 死路径 (目录不存在却无条件插 sys.path) → 存在性判定; **回归锁 4 个 (红/绿双向验证: 去掉修复全红, 恢复全绿)**; 安全项: options 已持有 / ast_cache 只存 JSON / 编译后不再改 compiler** | **✅ 同族清零 + 有锁** |
| **88** | **2026-09-08** | **L1** | **pyslang SourceManager 生命周期 — benchmark 波动真根因 (iter_185)** | **pr5 wrapper `test_l1_instance_chain` 失败 (L1=0), 按纪律查根因** | **真因定位 + 3 行修复** | **证伪 iter_181~184 的 "语料非 UTF-8" 归因 (axi/common_cells 0 个非 UTF-8 文件); 真因: `SVCompiler._do_compile` 把 `pyslang.SourceManager` 存成局部变量 → parse 循环结束被 GC → 源文件 buffer 释放 → 符号名/token 是指向释放内存的 `string_view`; 最小复现 4 模式 × 4 次 (私有 manager 丢引用 → top 名 UnicodeDecodeError, 保留引用 → 正常); 修复: `self._source_manager` 所有权 + `self._param_overrides` 同类加固 (slang 侧 topModules/paramOverrides 都是 string_view 容器); 效果: 符号 10,957→23,567 / 乱码名 3,349→0 / nodes 2,275→4,946 且 3/3 一致 / clk 0~137→445 / flakiness stdev=0.0 / pr5 1 failed→13 passed+1 skip; iter_184 基于错诊断放宽的断言已收紧; unit+regression 2111+35 subtests / truth+cli+integration 814** | **✅ 真根因闭环** |
| **87** | **2026-09-08** | **L1** | **getter 残余点 + 部分 elaboration 记录 (iter_184)** | **方豆 "继续"** | **崩溃消除 + 记录** | **修 `graph_builder.py:822` / `bit_select_handler.py:330` 两处 name getter 守护 → benchmark 崩溃 3/5→0/4; 当时记 "非 UTF-8 identifier → 部分 elaboration (nodes 1,076~3,408 / clk 0~137)" — **该归因已被 iter_185 证伪** (真因 = SourceManager 生命周期), 记录保留为历史; 借错误归因放宽的 clk 断言已在 iter_185 收紧** | **⚠️ 归因已更正** |
