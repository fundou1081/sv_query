...
| **115** | **2026-09-08** | **L1** | **修 fixture 尝试 -> 暴露依赖链, 已回退 (iter_212)** | 方豆: 先不处理可视化相关的 | **⚠️ 回退 + 关键发现** | **真因确认: `sim/test_comprehensive.sv` 输出端口 `output wire q` 被 `always_ff` 过程赋值 -> 19 个 AssignToNet; 修成 `output logic` 后 rc=1 -> rc=0 (方向正确); 但全量门禁 **18 -> 40 failed** (新增 24 个, 主要在 usage/test_coverage_generate.py) -> **那批测试的期望值固化了 partial AST 的输出** = 假绿有"记忆"; 按纪律回退到 18 failed 已知状态, 登记为专门批次 (修 fixture + 同批更新依赖断言)** | **⚠️ 回退 (发现已留档)** |
