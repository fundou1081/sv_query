...
|---|---|---|---|---|
| **unit** (95 文件) | 单模块/单函数 | — | 每个提取器/构建器/分析器**单元**行为正确 | 全绿 (计数见 INDEX 基线; iter_087 cache 序列化根因修复) |
...
| 命令 | 范围 | 角色 | 基线状态 (沙箱) |
|---|---|---|---|
| `pytest sim/tests/unit sim/tests/cli` | 95+46 | **主回归** | 全绿 (计数见 INDEX 基线; 沙箱 `~/.svq/cache` 不可写会假失败) |
