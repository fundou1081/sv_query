...
| 46 | `get_generate_instances` 覆盖率不一致: conditional+loop generate 场景返回 0 (M=2 + G=0) | semantic_adapter.get_generate_instances (iter_177 已移出 → legacy; 该行失效) | iter_056 R2 核实附带发现, 当时承诺"记入已知清单"但未登记 (iter_078 补记); conditional+loop generate 实例可能漏报, 影响 connection_extractor L123/L147 的 generate 实例补集 |
