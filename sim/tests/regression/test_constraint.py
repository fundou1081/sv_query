...
module top(output [7:0] data);
    packet p;
        ...
        # 验证: packet 类相关节点存在
        has_packet = any('packet' in n.lower() or 'addr' in n.lower() for n in nodes)
        self.assertTrue(has_packet or len(nodes) > 0, 
            f"constraint-related node not found in {nodes}")
