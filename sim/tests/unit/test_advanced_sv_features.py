...
'''
        ...
        print(f"✓ typedef struct handled correctly")
...
'''
        ...
        self.assertTrue(len(addr_nodes) > 0, f"addr node should exist")
        ...
        self.assertTrue(len(data_in_nodes) > 0, f"data_in node should exist")
        
        data_in_node = graph.get_node(data_in_nodes[0])
        self.assertEqual(data_in_node.width, (31, 0), f"data_in width should be (31,0)")
        
        print(f"✓ explicit parameter in module handled correctly")
        print(f"  ADDR_WIDTH=8 → addr[7:0] = {addr_node.width}")
        print(f"  Note: package::param resolution is separate issue")
...
'''
        ...
        print(f"✓ struct.member parameter access handled")
...
'''
        ...
        self.assertTrue(len(result_nodes) > 0, f"result/out node should exist")
        ...
        self.assertGreater(len(a_to_gray_edges), 0, f"function inputs should drive gray_conv")
        
        print(f"✓ static function in module handled correctly")
...
'''
        ...
        self.assertGreater(len(dut_nodes), 0, f"Module dut should be parsed")
        
        # Verify data ports exist with correct widths
        data_in_nodes = [n for n in dut_nodes if 'data_in' in n]
        self.assertTrue(len(data_in_nodes) > 0, f"data_in port should exist")
        
        data_in_node = graph.get_node(data_in_nodes[0])
        self.assertEqual(data_in_node.width, (7, 0), f"data_in width should be (7,0)")
        
        # Verify outputs
        parity_nodes = [n for n in dut_nodes if 'parity_out' in n]
        self.assertTrue(len(parity_nodes) > 0, f"parity_out should exist")
        
        print(f"✓ package with functions handled correctly")
...
'''
        ...
        self.assertTrue(len(tx_keep_nodes) > 0, f"tx_keep node should exist")
        ...
        print(f"✓ complex parameter expressions handled correctly")
        print(f"  KEEP_WIDTH = (DATA_WIDTH/8) = 8 → tx_keep[7:0]")
...
'''
        ...
        print(f"✓ parameter reference handled")
