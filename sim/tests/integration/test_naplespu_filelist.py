...
def test_naplespu_stats_filelist():
    ...
    r = _run("stats", "--filelist", FILELIST_PATH)
...
def test_naplespu_risk_filelist():
    ...
    r = _run("risk", "analyze", "--filelist", FILELIST_PATH)
...
def test_naplespu_cdc_filelist():
    ...
    r = _run("cdc", "analyze", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_sva_extract_filelist():
    """sva extract"""
    r = _run("sva", "extract", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_sva_coverage_filelist():
    ...
    r = _run("sva", "coverage", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_sva_timing_filelist():
    """sva timing"""
    r = _run("sva", "timing", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_timing_filelist():
    """timing analyze: 关键路径"""
    r = _run("timing", "analyze", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_verify_gap_filelist():
    ...
    r = _run("verify", "gap", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_coverage_suggest_filelist():
    """coverage suggest"""
    r = _run("coverage", "suggest", "-s", "synchronizer.sync0", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_controlflow_filelist():
    ...
    r = _run("controlflow", "analyze", "synchronizer.sync0", "--filelist", FILELIST_PATH)
...
def test_naplespu_dataflow_filelist():
    ...
    r = _run("dataflow", "analyze", "synchronizer.sync0", "synchronizer.data_o", "--filelist", FILELIST_PATH)
    assert r.returncode == 0
...
def test_naplespu_visualize_filelist():
    ...
    r = _run("visualize", "graph", "--html", out_html, "--filelist", FILELIST_PATH)
