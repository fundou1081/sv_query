...
def fix_imports_cmd(
    ...
    # 文本输出
    typer.echo(f"=== Fix Imports Report ===\n")
    ...
    typer.echo(f"=== Identifiers Needing Fix ===\n")
    for sug in result["identifiers"][:top]:
        ...
        if sug["fixable"]:
            ...
            typer.echo(f"    Found in: (not in project — NaplesPU 自身缺定义或宏/系统函数)")
    ...
    if fixable and write:
        try:
            ...
            with open(write, "w") as f:
                f.write(f"# Generated by sv_query fix imports\n")
