...
class SVAExtractor:
    ...
    def _parse_sequence(self, node, prefix: str = '') -> Optional[SVASequenceNode]:
        """解析 SequenceSymbol"""
        name = str(getattr(node, 'name', '')).strip()
        if not name:
            return None
    ...
    def _parse_property(self, node, prefix: str = '') -> Optional[SVAPropertyNode]:
        """解析 PropertySymbol"""
        name = str(getattr(node, 'name', '')).strip()
        if not name:
            return None
    ...
    def _parse_assertion_syntax(self, syntax_node, graph: SVAGraph, prefix: str = ''):
        ...
        if assertion_kind:
            ...
            graph.assertions.append(SVAAssertionNode(
                id=assertion_id,
                kind=assertion_kind,
    ...
    def _find_property_ref(self, node) -> str:
        ...
        for child in self._iter_children(node):
            ck = str(getattr(child, 'kind', ''))
            if 'PropertySpec' in ck or 'PropertyExpr' in ck or 'SequenceExpr' in ck or 'AssertProperty' in ck or 'AssumeProperty' in ck or 'CoverProperty' in ck:
                ref = self._find_property_ref(child)
                if ref:
                    return ref
