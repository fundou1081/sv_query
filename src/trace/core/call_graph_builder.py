...
from .graph.call_graph_models import CallGraph, CallNode
...
class CallGraphBuilder:
    ...
    def build(self, entry_class: str, entry_method: str) -> CallGraph:
        ...
        if entry_node is None:
            return CallGraph(entry_point=entry_key, errors=[f"未找到入口: {entry_key}"])
    ...
    def _find_invocation(self, node) -> object | None:
        """在 ExpressionStatement 中找 InvocationExpression"""
        return self._find_node(node, "Invocation")

    def _find_randomize(self, node) -> object | None:
    ...
    def _find_node(self, node, kind_keyword: str) -> object | None:
        """递归查找包含指定关键词的节点"""
        kind = str(getattr(node, "kind", ""))
    ...
    def _process_randomize(self, node, caller: str) -> CallNode | None:
        """处理 randomize 表达式
