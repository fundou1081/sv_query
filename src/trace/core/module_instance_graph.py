...
class ModuleInstanceGraph:
    ...
    def _get_generate_block_name(self, inst) -> str | None:
    ...
    def _find_all_hierarchy_instantiations(self, node, result_list, parent_path=""):
        ...
        should_recurse = (
            ("CompilationUnit" in kind_str)
            or ("ModuleDeclaration" in kind_str)
            or ("GenerateRegion" in kind_str)
            or "LoopGenerate" in kind_str
            or "IfGenerate" in kind_str
            or "CaseGenerate" in kind_str
        )
        if should_recurse:
                ...
                if hasattr(separated_list, "__iter__"):
                    for elem in separated_list:
                        ...
                        if declarator and hasattr(declarator, "name") and declarator.name:
                            port_name = (
                                declarator.name.value if hasattr(declarator.name, "value") else str(declarator.name)
                            )
    ...
    def _extract_port_info(self, port_decl, module_name: str) -> PortInfo | None:
    ...
    def _get_module_name_from_type(self, node) -> str | None:
    ...
    def _extract_module_instantiation(self, node, parent_path: str):
        ...
        for elem in instances_list:
            ...
            if instance_id not in self.instances:
                instance_node = ModuleInstanceNode(id=instance_id, module_type=module_type, parent=parent_path or None)
