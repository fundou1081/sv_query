...
class GraphTraversal:
    ...
    def _find_by_edge_kind(self, signal_id: str, src_or_dst: str, edge_kind: EdgeKind) -> list[any]:
        """
        按边类型查找相关节点
        ...
        for src, dst in list(self.graph.edges()):
            ...
            if node and node.id not in seen_ids:
                ...
                if other_id in self.graph.nodes():
                    sub_results = self._collect_all(other_id, direction, edge_kind, seen_ids)
                    results.extend(sub_results)

        return results

    def _collect_all_by_edge_kind(
        self, signal_id: str, direction: str, edge_kind: EdgeKind, seen_ids: set[str] | None = None
    ) -> list[any]:
        """
        按边类型递归收集（带边类型检查的递归）
        ...
        for src, dst in list(self.graph.edges()):
            ...
            if edge.kind == edge_kind:
                ...
                if node and node.id not in seen_ids:
                    ...
                    # 递归追溯
                    if other_id in self.graph.nodes():
                        sub = self._collect_all_by_edge_kind(other_id, direction, edge_kind, seen_ids)
                        results.extend(sub)
