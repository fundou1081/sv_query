...
class SubroutineExpander:
    ...
    def expand(self, call_site: CallSiteInfo, ctx: Dict = None) -> ExpansionResult:
        ...
        for branch in branches:
            ...
            if not signal_mapping and not cond_str:
                func_return_id = f"{call_site.lhs_name.split('.')[0] if '.' in call_site.lhs_name else 'top'}.{call_site.call_name}"
                result.nodes.append(TraceNode(
                    id=func_return_id, name=call_site.call_name,
    ...
    def _collect_case_branches(self, stmt: Any, func_name: str, branches: List[BranchInfo],
                                parent_condition: Any = None, in_else: bool = False,
                                case_expr: Any = None):
        """收集 case 语句的分支
