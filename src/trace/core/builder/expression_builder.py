...
from trace.core.graph.models import (
    NodeKind, TraceNode, EdgeKind, SignalGraph, TraceEdge
)
...
class ExpressionBuilder:
    ...
    def _create_node(self, node_id: str, name: str, kind: NodeKind,
                     **kwargs) -> TraceNode:
        """创建节点并添加到图中"""
        # 提取已知参数，避免重复
    ...
    def build_expression(self, operands: List[str], expression: str,
                         result: str, module: str = "") -> Optional[str]:
        """构建表达式节点
    ...
    def build_function_call(self, function_name: str, arguments: List[str],
                           result: str, module: str = "") -> Optional[str]:
        """构建函数调用节点
    ...
    def build_conditional(self, condition: str, true_branch: str,
                          false_branch: str, result: str,
