...
class ASTCache:
    ...
    def get_by_key(self, cache_key: str, force: bool = False) -> Optional[Dict]:
        ...
        try:
            ...
            if data.get("version") != CACHE_VERSION:
                logger.debug(f"Cache version mismatch, rebuilding...")
                return None
            
            # 验证 sources hash
            if data.get('sources_hash') != cache_key:
                logger.debug(f"Cache hash mismatch, rebuilding...")
                return None
    ...
    def get(self, source_file: str, force: bool = False) -> Optional[Dict]:
