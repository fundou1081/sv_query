...
class Branch:
    ...
    value: Optional[str] = None  # case 的值 "0", "1", "default"
...
class ControlBlock:
    ...
    def has_else(self) -> bool:
        """是否有 else 分支"""
        return any(not b.condition for b in self.branches) or \
               any(b.value == "default" for b in self.branches)
...
class ControlFlowResult:
    ...
    # 控制块信息
    control_block: Optional[ControlBlock] = None
    ...
    # Z3 分析 (如果有矛盾条件)
    z3_analysis: Optional[Z3Result] = None
    
    # 警告
    warnings: List[LintWarning] = field(default_factory=list)
    
    # === 死锁/可达性 ===
    state_machine_analysis: Optional[StateMachineAnalysis] = None
