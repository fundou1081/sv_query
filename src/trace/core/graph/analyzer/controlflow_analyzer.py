...
class ConditionedDriver:
    ...
    def conditions(self) -> list[ConditionInfo]:
        """获取所有条件"""
        return [ConditionInfo(expr=e.condition, edge=e) for e in self.edges if e.condition]
...
class ControlFlowAnalyzer:
    ...
    def __init__(self, graph_builder: GraphBuilder | None = None):
    ...
    def analyze(self, signal: str) -> ControlFlowAnalysis:
        ...
        for _to_node, edges in by_source.items():
            ...
            if conditional_edges:
                result.conditioned_drivers.append(ConditionedDriver(to_node=signal, edges=conditional_edges))
    ...
    def find_conditioned_signals(self, module: str | None = None) -> list[str]:
        """
        找出模块中所有有条件驱动的信号
