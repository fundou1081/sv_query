...
class ControlFlowGraph:
    ...
    def get_control_block(self, ast_node) -> Optional[ControlBlock]:
        """根据 AST 节点获取控制块"""
        return self._blocks.get(id(ast_node))
    
    def get_state_machine(self, state_var: str) -> Optional[StateMachineAnalysis]:
        """获取状态机信息"""
        # 查找所有 case(state_var) 的节点
        case_nodes = [
            n for n in self._condition_nodes
            if n.condition_vars and state_var in n.condition_vars
            and n.kind == ControlFlowNodeKind.CONDITION
        ]
