...
class PyslangAdapter:
    ...
    def clean_name(self, name) -> str:
        ...
        [P0-1 2026-06-13] 委托给 _safe.clean_name (单一规范实现)。
        """
    ...
    def _get_parser_for_module(self, module) -> Any | None:
    ...
    def get_port_name(self, port) -> str | None:
        """获取端口名称"""
    ...
    def _get_declarations_by_type(self, module, decl_type: str) -> list:
        ...
        for member in module.members:
            if (
                hasattr(member, "kind")
                and member.kind.value
                == getattr(
                    SyntaxKind,
                    decl_type.replace("PortDeclaration", "PortDeclaration").replace(
        ...
        for port in node.port_connections:
            if hasattr(port, "port") and hasattr(port, "signal"):
                self.connections.append(
                    (module_name, self._get_signal_name(port.port), self._get_signal_name(port.signal))
                )
